...
export class BooksService {
    ...
    async storeBook(bookDTO: StoreBookDto, book: Express.Multer.File, user: JwtPayloadType) {
        const existingBook = await this.getBookByTitle(bookDTO.title)
        if (existingBook) {
            throw new BadRequestException("Book already exists")
        }
        const bookURL = await this.dropBoxService.storeFile(book, bookDTO.title)
        const newBook = await this.db.book.create({
            data: { ...bookDTO, bookURL, userId: user.sub, dateUploaded: new Date(), dateAuthored: new Date(bookDTO.dateAuthored), pageCount: Number(bookDTO.pageCount), isMature: Boolean(bookDTO.isMature) }
        })
        return newBook
    }
    ...
    async updateBook(id: string, userId: number, bookDTO: UpdateBookDto, book: Express.Multer.File) {
        if (!bookDTO && !book) {
        ...
        const updatableFields = ['isMature', 'bookURL', 'author', 'pageCount', 'title', 'dateAuthored'];

        let bookURL: string | undefined
        if (book) {
            bookURL = await this.dropBoxService.storeFile(book, bookDTO.title || bookRecord.title)
        }
        updatableFields.forEach(field => {
            if (field !== "bookURL" && bookDTO[field] !== undefined) {
                updateData[field] = field === 'dateAuthored'
                    ? new Date(bookDTO.dateAuthored as unknown as string)
                    : (field === 'bookURL' ? bookURL : bookDTO[field]);
            }
        });
